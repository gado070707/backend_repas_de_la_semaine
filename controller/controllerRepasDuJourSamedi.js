...
async function verification_de_la_presence_du_nom_de_la_viande_dans_la_bdd_table_repasdujour(req_nom){
    try {
        ...
        var query = "select r.id_viandes AS VIANDES FROM `repas du jour` r INNER JOIN `viandes` v ON v.id = r.id_viandes INNER JOIN `jours` j ON j.id = r.id_jours WHERE j.nom LIKE 'Samedi' AND r.id_viandes in ( SELECT v.id FROM viandes v WHERE v.nom LIKE '"+req_nom+"')";
        var bdd_nom = await conn.query(query);
