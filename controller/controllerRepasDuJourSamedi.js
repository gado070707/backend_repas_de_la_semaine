...
const afficherRepasDuJourSamedi = (async function (req, res, next){    
  ...
  try {
      ...
      var query = "select v.nom AS VIANDES FROM `repas du jour` r INNER JOIN `viandes` v ON v.id = r.id_viandes INNER JOIN `jours` j ON j.id = r.id_jours WHERE j.nom LIKE 'Samedi'";
...
const supprimerRepasDuJourSamedi = (async function (req, res, next){    
    let conn;
    ...
    // Check mandatory request parameters
        ...
        verification_de_la_presence_du_nom_de_la_viande_dans_la_table_viandes(req.body.nom).then(function (nom_bdd) { 
            ...
            if (nom_bdd == req.body.nom) {
                ajouterRepasDuJourSamedi(req.body.nom);
                return res.status(200).json({ error: 'La repas sera ajouté à samedi' });
                // return res.status(409).json({ error: 'La repas sera ajouté à samedi' });
                // return 90;
            }
            else{
...
async function ajouterRepasDuJourSamedi(reponse){
    ...
    verification_de_la_presence_du_nom_de_la_viande_dans_la_bdd_table_repasdujour(reponse).then(async function(data) {
        ...
        if(data == null){
            ...
            try {
                conn = await pool.getConnection();
                var query = "INSERT INTO `repas du jour`(`id_jours`, `id_viandes`) VALUES ((SELECT id FROM jours WHERE nom = 'Samedi'),(SELECT id FROM viandes WHERE nom LIKE '"+reponse+"'));";
...
async function verification_de_la_presence_du_nom_de_la_viande_dans_la_bdd_table_repasdujour(req_nom){
    try {
        ...
        var query = "select r.id_viandes AS VIANDES FROM `repas du jour` r INNER JOIN `viandes` v ON v.id = r.id_viandes INNER JOIN `jours` j ON j.id = r.id_jours WHERE j.nom LIKE 'Samedi' AND r.id_viandes LIKE ( SELECT v.id FROM viandes v WHERE v.nom LIKE '"+req_nom+"')";
        var bdd_nom = await conn.query(query);
