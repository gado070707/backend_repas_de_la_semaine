...
const supprimerRepasDuJourVendredi = (async function (req, res, next){    
    let conn;
    ...
    // Check mandatory request parameters
        ...
        verification_de_la_presence_du_nom_de_la_viande_dans_la_table_viandes(req.body.nom).then(function (nom_bdd) { 
            ...
            if (nom_bdd == req.body.nom) {
                ...
                return res.status(200).json({ error: 'La repas sera ajouté à vendredi' });
                // return res.status(409).json({ error: 'La repas sera ajouté à vendredi' });
                // return 90;
            }
            else{
...
async function verification_de_la_presence_du_nom_de_la_viande_dans_la_bdd_table_repasdujour(req_nom){
    try {
        ...
        var query = "select r.id_viandes AS VIANDES FROM `repas du jour` r INNER JOIN `viandes` v ON v.id = r.id_viandes INNER JOIN `jours` j ON j.id = r.id_jours WHERE j.nom LIKE 'Vendredi' AND r.id_viandes LIKE ( SELECT v.id FROM viandes v WHERE v.nom LIKE '"+req_nom+"')";
        var bdd_nom = await conn.query(query);
